...
def retrofit(docVecs, lexicon, numIters,beta):
    #count = 0
    ...
    for it in range(numIters):
        ...
        for doc in loopVocab:
            ...
            betaVec = np.zeros(docVecs.dimension)
            ...
            # doubling the weight
            newVec = docVecs.paper_embeddings[docVecs.paper2id[doc]]
            # loop over neighbours and add to new vector (currently with weight 1)
            # increased weight to 2
            for dindex,ppWord in enumerate(docNeighbours):
                betaVec += newDocVecs.paper_embeddings[docVecs.paper2id[ppWord]]           
            betaVec = betaVec / (numNeighbours)
                #betaVec = betaVec / (2*(dindex+1))  
                
#newVec = newVec + loopWeight*numNeighbours*newDocVecs.paper_embeddings[docVecs.paper2id[ppWord]]
            #betaVec = normalize(betaVec[:,np.newaxis], axis=0).ravel()
            newDocVecs.paper_embeddings[docVecs.paper2id[doc]] = (1-beta)*newVec + (beta*betaVec)
